...
wDup.data.columns[4].captions = ["Простой, %(ед.)"];
wDup.data.columns[5].captions = ["Неработосп., %(ед.)"];
...
$('#table-' + w.general.renderTo + ' tr > td:nth-child(5)').each(function(i, td) {
        let cell_value = +(parseInt(td.innerText.split('%')[0]));
...
$('#table-' + w.general.renderTo + ' th:last-child')
.css({
    "width":"23%",
    'padding-left' : '10px',
    'word-break':'normal',
...
// заменяем точки на запятые в колонках 3-5
[3,4,5].forEach(function(j) {

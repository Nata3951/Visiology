const filterGuid = '6a680a9c78b44ecaa2c7937ba28f32b3';
...
if (w.data.values[1].length >= 8) {
  const oniTotal = oniTotalMap[w.data.rows[7][1]] || w.data.values[1][0];
  w.general.text = `<span style="color:#212121;font-weight:bold;padding-left:0px">${valueName}</span> | <span style="color:#212121;font-weight:bold">${valueSecondName}</span> | Доля в портфеле отрасли: <span style="color:#212121;font-weight:bold">${(w.data.values[0][7] / oniTotal * 100).toFixed(2).split('.').join(',')}%</span>`;
} else {
